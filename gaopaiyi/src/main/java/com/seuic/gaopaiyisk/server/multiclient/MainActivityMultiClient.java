...
import com.seuic.gaopaiyisk.BuildConfig;
...
public class MainActivityMultiClient extends AppCompatActivity implements CompleteCallback{
...
//    private static int HEIGHT = 1568;
    ...
    private ServerForMultiClient csServer;
...
//        server = new GaopaiyiServer(this);
        new Thread(new Runnable() {
            @Override
            public void run() {
//                csServer = new ServerForMultiClient();
                csServer = new ServerForMultiClient(MainActivityMultiClient.this);
                csServer.initServer();
            }
        }).start();
    }
...
//        HEIGHT = AppScreenMgr.getScreenHeight(this);
    ...
    public void onComplete(CommodityInfo commodityInfo) {
        if (commodityInfo != null) {
            ...
            /*if (csServer.mClientList.size() > 0) {
                csServer.sendMessage(s);
            }*/
            if (callback!=null){
                callback.sendMsg(s);
            }
    ...
    private void showToast(final String msg) {
        try {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MainActivityMultiClient.this, msg, Toast.LENGTH_SHORT).show();
                }
            });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private SendData2ClientCallback callback;

    public void initCallback(SendData2ClientCallback callback){
        this.callback = callback;
    }
    public interface SendData2ClientCallback{
